  ...
  head: ({ params }) => {
    ...
    return {
      meta: [
        ...
        { name: "keywords", content: getMetaTranslation("meta.parts.keywords", locale) },
  ...
  const { data: parts = staticParts, isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return apiParts.map((ap) => ({
        ...
        oem:
          typeof ap.oem === "string" && ap.oem.startsWith("{")
            ? (() => {
                try {
                  return JSON.parse(ap.oem);
                } catch {
                  return ap.oem;
                }
              })()
    ...
    <PageShell>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          {currentCategory && (
            <Link to={localePath("/parts")} className="font-bold text-rocsta-green hover:underline">
              {t("parts.viewAllCats")}
            </Link>
          )}
        </div>
